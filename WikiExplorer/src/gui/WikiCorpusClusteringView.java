...
public class WikiCorpusClusteringView extends JApplet {
...
//	Factory<Graph<Number,Number>> graphFactory;
    ...
    public void open(HashMap<String, String> _iwlinks,
            String[] _links,
            String[] _catMembs,
            JPanel jpNETWORK,
            String[] a) throws IOException {

        centerPage = new WikiNode(a);
        links = _links;
        iwlinks = _iwlinks;
        catMembs = _catMembs;

        mode = 2;
        jpNETWORK.add(this, BorderLayout.CENTER);

        start();
        // Add a restart button so the graph can be redrawn to fit the size of the frame

    }
    static HashMap<String, String> iwlinks;
    static String[] links;
    static String[] catMembs;

    public static void main(String[] args) throws IOException {

        WikiCorpusClusteringView cd = new WikiCorpusClusteringView();
        cd.start();
        // Add a restart button so the graph can be redrawn to fit the size of the frame
    ...
    public void start() {
        ...
        try {
            ...
            ex.printStackTrace();
            Logger.getLogger(WikiCorpusClusteringView.class.getName()).log(Level.SEVERE, null, ex);
        }

    }

    public Graph loadDemoGraph() {
...
//        }
    ...
    private Graph<Number, Number> loadWikiGraph() {
        Graph<Number, Number> graph = getGraph();
        return graph;
    }
    
    WikiNode centerPage = null;
    Hashtable<Integer, WikiNode> linksHASH = new Hashtable<Integer, WikiNode>();
    Hashtable<WikiNode, Integer> linksHASH_INV = new Hashtable<WikiNode, Integer>();

    public Graph<Number, Number> getGraph() {

        UndirectedGraph<Number, Number> g1 = new UndirectedSparseMultigraph<Number, Number>();


        // KERN
        TreeSet<Number> s0 = new TreeSet<Number>();
        s0.add(0);
        colorCluster(s0, Color.BLACK);
        linksHASH.put(0, centerPage);
        g1.addVertex(0);

        int i = 1;
        
        // add direct linked pages
        TreeSet<Number> s1 = new TreeSet<Number>();
        for (String l : links) {

            WikiNode n = new WikiNode(centerPage.wiki, l);
            linksHASH.put(i, n);
            linksHASH_INV.put(n, i);
        ...
        colorCluster(s1, Color.BLUE);
        

        // add IWL-Pages
        TreeSet<Number> s2 = new TreeSet<Number>();
        for (String key : iwlinks.keySet()) {
            String value = iwlinks.get(key);
            System.out.println("val=" + key + "_" + value);
            WikiNode n = new WikiNode(key, value);
            linksHASH.put(i, n);
            linksHASH_INV.put(n, i);
            s2.add(i);
            g1.addVertex(i);
            if (!linksHASH_INV.containsKey(value)) {
                g1.addEdge(i, 0, i);
            }
            i++;
        }    
        colorCluster(s2, Color.RED);
        
        // add Categorie Members
        TreeSet<Number> s3 = new TreeSet<Number>();
        for (String l : catMembs) {

            WikiNode n = new WikiNode(centerPage.wiki, l);

            g1.addVertex(i);
            if (linksHASH_INV.containsKey(l)) {
                g1.addEdge(i, 0, i);
                linksHASH.put(i, n);
            }
            i++;
